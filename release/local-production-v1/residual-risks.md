...
| L8-R02 | Extended real-project usability and conversational friction are not yet known. | High | Blocks final local-v1 acceptance, not automated candidate handoff. Execute the owner campaign in `extended-testing-plan.md`. |
| L8-R03 | Native Codex or Claude Code behavior may differ from CLI probes or change after an application update. | Medium | Explicit owner-observed native-app smoke remains required. Retain manual fallback and re-test after provider changes. |
...
| L8-R05 | Compatible dependency resolution can drift between installations of the exact wheel. | Medium | L9 records the resolved primary inventory; use a controlled cache or constraints when repeatability requires it. |
