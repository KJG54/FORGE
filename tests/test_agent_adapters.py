...
def test_missing_preferred_adapter_degrades_explicitly_to_manual(tmp_path: Path) -> None:
    ...
    configured = initialized.configuration.model_copy(
        update={
            "agents": initialized.configuration.agents.model_copy(
                update={"preferred_adapter": "unregistered"}
            )
        }
    )
    ...
    assert selected.requested_adapter_id == "unregistered"
    assert selected.adapter.adapter_id == "manual"
    assert selected.fallback_reason == (
        "Adapter 'unregistered' is not registered; using manual handoff"
    )
    ...
    assert prepared.selection.requested_adapter_id == "unregistered"
...
def test_agent_doctor_and_handoff_cli_expose_manual_selection(tmp_path: Path) -> None:
    ...
    doctor = runner.invoke(
        app,
        ["agent", "doctor", "--adapter", "unregistered", "-C", str(initialized.layout.root)],
    )
    assert doctor.exit_code == 0, doctor.stdout
    assert "Requested adapter: unregistered" in doctor.stdout
    assert "Selected adapter: manual" in doctor.stdout
    assert "Fallback: Adapter 'unregistered' is not registered" in doctor.stdout
