...
def test_m2_increment_12_does_not_contain_later_milestone_modules() -> None:
    package = ROOT / "src" / "forge"
    deferred = {"agents", "capabilities"}
    present = sorted(name for name in deferred if (package / name).exists())
    assert not present, f"M2 Increment 12 created deferred implementation modules: {present}"
